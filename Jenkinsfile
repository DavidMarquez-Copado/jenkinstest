  ...
  stages {
    ...
    stage('coverage') {
      steps {
        ...
        main '"-f" ${COPADO_COVERAGE_FEATURE_BRANCH} "-i" ${COPADO_COVERAGE_CLIENT_ID} "-s" ${COPADO_COVERAGE_CLIENT_SECRET} "-u" ${COPADO_COVERAGE_USER_NAME} "-p" ${COPADO_COVERAGE_PASSWORD}'
      }
    }
  }
  post {
